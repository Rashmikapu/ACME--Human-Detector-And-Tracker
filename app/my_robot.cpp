...
perception::MyRobot::MyRobot() : K((cv::Mat_<double>(3, 3) << 600.0, 0, 320.0,
0, 600.0, 240.0, 0, 0, 1.0)),
R((cv::Mat::eye(3, 3, CV_64F))), T((cv::Mat_<double>(3, 1) << 0, 0,
       2.0))  {
  // Sample intrinsic matrix K
  // K = (cv::Mat_<double>(3, 3) << 600.0, 0, 320.0, 0, 600.0,
  // 240.0, 0, 0, 1.0);
...
void perception::MyRobot::TransformToRobotFrame(
    const std::vector<cv::Rect> &detected_objects, double *X,
    double *Y, double *Z) {
  ...
  for (const cv::Rect &bbox : detected_objects) {
    ...
    cv::Mat normalized_point = (cv::Mat_<double>(3, 1) <<
    center.x, center.y);
    ...
    if (X) *X = camera_frame_point.at<double>(0, 0);
    if (Y) *Y = camera_frame_point.at<double>(1, 0);
    if (Z) *Z = camera_frame_point.at<double>(2, 0);
  }
}
