...
cv::Mat image3 = cv::imread(
      "../../data/sample.jpeg");
/**
 * @brief Construct a new TEST object, This object test the bounding box update creation
 * 
 */
TEST(VisualizationTest, CreateBoundingBoxTest) {
    ...
    // Call the function
    perception::Visualization::createBoundingBox(indices1, boxes1, &bboxes1, image3, class_list1, class_ids1, confidences1);

    // Add assertions to check if the function behaved as expected
    // For example, you can check if bboxes vector contains the expected Rect2d values
