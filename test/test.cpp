...
// EXPECT_EQ(my_detector.postProcess(input_image,detections),cv::Mat::zeros(1, 1, CV_64F));
...
TEST(preprocess_test, this_will_pass) {
        EXPECT_FALSE(detections.empty());
}

TEST(postprocess_test, this_will_pass) {
EXPECT_TRUE(cv::countNonZero(resultMat != expectedMat) == 0);
}